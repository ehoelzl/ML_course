from costs import compute_mse_loss, compute_logistic_loss, compute_reg_logistic_loss_l2, compute_reg_logistic_loss_l1
...
def reg_logistic_regression_sgd(y, tx, lambda_, initial_w, max_iters, gamma, _print=True):
    """
    Regularized (using L2 regularization) Logistic regression using gradient descent or SGD
    ...
    for n_iter, data in enumerate(batch_iter(y, tx, batch_size, num_batches=max_iters)):
        # Fetch batch data
        y_batch, tx_batch = data
        dw = compute_logistic_gradient(y_batch, tx_batch, w) + (lambda_ * w)
        loss = compute_reg_logistic_loss_l2(y_batch, tx_batch, w, lambda_)
        # Update parameters
        w = w - gamma * dw
        # store w and loss
        if n_iter % 1000 == 0 and _print:
...
def reg_logistic_regression_l1(y, tx, lambda_, initial_w, max_iters, gamma, _print=True, decay=False):
    """
    Regularized (using L2 regularization) Logistic regression using gradient descent or SGD
